/**
 * Watchlist scoring — pure functions only.
 *
 * ScoringInputs, scoreMomentum, and scoreTechnical are pure functions with no
 * external I/O dependencies, so they can be imported freely in unit tests.
 *
 * Orchestration (computeWatchlistScores, ComputeResult, detectMarket,
 * classifyQuality) lives in watchlist-orchestrator.ts.
 * Indicator math (computeIndicators, ComputedIndicators) lives in
 * watchlist-indicators.ts.
 * Yahoo glue (buildScoringInputs) lives in watchlist-fetcher.ts.
 */
