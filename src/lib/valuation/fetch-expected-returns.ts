/**
 * Expected Returns data fetcher — extracted from
 * src/app/api/valuation/expected-returns/route.ts so it can be called
 * directly during SSR from /markets without going through HTTP.
 *
 * Computes current valuations + tactical overlay for SPY/Gold/BTC/Bonds
 * using Yahoo Finance + FRED + an in-house gold-price helper.
 */
...
const LOG_SRC = "lib/valuation/fetch-expected-returns";
...
async function fetchM2(): Promise<number | null> {
  ...
  try {
    ...
    if (!res.ok) {
      logger.error(LOG_SRC, `FRED M2 error: ${res.status}`);
      return null;
    }
    ...
    logger.error(LOG_SRC, "Error fetching M2", { error });
    return null;
  }
}
...
async function fetchQuote(symbol: string): Promise<{
  ...
  try {
    ...
    logger.error(LOG_SRC, `Error fetching ${symbol}`, { error });
    return null;
  }
}
...
async function fetchHistoricalData(
  ...
  try {
    ...
    logger.error(LOG_SRC, `Error fetching history for ${symbol}`, { error });
    return null;
  }
}
...
function getVixBias(vix: number): "bullish" | "neutral" | "bearish" {
  if (vix > 35) return "bullish";
  if (vix > 25) return "neutral";
  if (vix < 12) return "bearish";
  return "neutral";
}
...
function calculateTacticalSummary(assets: AssetValuation[]): TacticalSummary {
  ...
  if (alignment === "strong-buy") {
    message = `Strong setup: ${aligned
      .filter(
        (s) => assets.find((a) => a.symbol === s)?.valuation.zone === "cheap",
      )
      .join(", ")} cheap and trending up`;
...
export async function fetchExpectedReturnsData(): Promise<ExpectedReturnsResponse> {
  const assets: AssetValuation[] = [];

  const [spyData, gspcData, goldData, btcData, tnxData, vixData, irxData, m2] =
    await Promise.all([
      fetchQuote("SPY"),
      fetchQuote("^GSPC"),
      getGoldPrice(),
  ...
  if (goldData && m2) {
    ...
    assets.push({
      ...
      expectedReturn: { tenYear: 3, confidence: "low" },
      tactical: {
        vs200dma,
        momentum: determineMomentum(vs200dma),
        vs50dma,
  ...
  if (tnxData) {
    ...
    const tactical: TacticalOverlay = {
      ...
      bias,
      note:
        yieldCurveSlope !== undefined && yieldCurveSlope < 0 ? "inverted" : note,
    };
