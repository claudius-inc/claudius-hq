...
export function StockFilters({ reports }: StockFiltersProps) {
  ...
  const [activeTab, setActiveTab] = useState<"ticker" | "thematic">("ticker");
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex gap-1 bg-gray-100 p-1 rounded-lg">
        ...
        >
          By Ticker
          <span className="ml-1.5 text-xs text-gray-400">({sortedTickers.length})</span>
        </button>
        <button
        ...
        >
          Thematic
          <span className="ml-1.5 text-xs text-gray-400">({thematicReports.length})</span>
        </button>
      </div>
      ...
      {activeTab === "thematic" && (
        <div className="space-y-4">
          {thematicReports.length > 0 ? (
            ...
            <div className="bg-white border border-gray-200 rounded-xl p-12 text-center">
              <h3 className="text-lg font-semibold text-gray-900 mb-1">No thematic reports</h3>
              <p className="text-sm text-gray-500">
                Thematic, comparison, and weekly scan reports will appear here
              </p>
            </div>
          )}
        </div>
