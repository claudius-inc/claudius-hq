...
import { SectorData, formatPercent, getPercentColor, getPercentBg, getHoldingsUrl } from "./index";

function TrendArrow({ trend }: { trend: SectorData["momentum_trend"] }) {
...
function relStrengthTooltip(value: number | null): string | undefined {
  if (value === null) return undefined;
  return `vs SPY: ${value >= 0 ? "+" : ""}${value.toFixed(1)}%`;
}
        ...
        <table className="min-w-full divide-y divide-gray-100">
          <thead className="bg-gray-50">
            <tr>
              ...
              <th className={`${thBase} text-left`}>Sector</th>
              ...
              <tr key={s.id} className="hover:bg-gray-50">
                ...
                >
                  <div className="flex items-center justify-end gap-1 cursor-default">
                    <TrendArrow trend={s.momentum_trend} />
                    <span>{formatPercent(s.change_1w)}</span>
                  </div>
                </td>
                ...
                <td className="px-3 py-2.5 whitespace-nowrap">
                  <div className="flex items-center gap-1">
                    ...
                    <a href={`https://www.tradingview.com/chart/?symbol=${s.ticker}`} target="_blank" rel="noopener noreferrer" className="p-1 text-gray-400 hover:text-gray-600 inline-block" title="View chart on TradingView">
