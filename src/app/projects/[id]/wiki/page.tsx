...
export default async function WikiIndexPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const pagesRes = await db.execute({
    sql: "SELECT id, project_id, slug, title, sort_order, created_at, updated_at FROM wiki_pages WHERE project_id = ? ORDER BY sort_order ASC",
    args: [Number(id)],
  });
  const pages = pagesRes.rows as unknown as WikiPage[];
    ...
    <div className="min-h-screen">
      ...
      <main className="max-w-7xl mx-auto px-4 py-6">
        ...
        <div className="flex items-center gap-3 mb-6">
          <h1 className="text-2xl font-bold text-gray-900">📖 {project.name} Wiki</h1>
        ...
        {pages.length === 0 ? (
          <div className="card p-8 text-center text-gray-400">
            ...
            <p className="text-sm">Use the API to import wiki content for this project.</p>
          </div>
        ) : (
          <div className="grid gap-3">
            {pages.map((page, idx) => (
              <Link
                key={page.slug}
                href={`/projects/${id}/wiki/${page.slug}`}
