...
export async function generateMetadata(props: { params: Promise<{ id: string }> }): Promise<Metadata> {
  ...
  try {
    ...
    if (result.rows.length > 0) {
      ...
      return { title: `${project.name} - Distribution Plan` };
    }
  } catch {}
  return { title: "Distribution Plan" };
}

export default async function DistributionPlanPage(props: { params: Promise<{ id: string }> }) {
  const params = await props.params;
  const { id } = params;
  ...
  const htmlContent = project.plan_distribution 
    ? marked(project.plan_distribution, { async: false }) as string
    : null;
  ...
  return (
    <>
      ...
      <div className="flex items-center gap-2 text-sm text-gray-400 mb-4 flex-wrap">
        ...
        <span>/</span>
        <span className="text-gray-700">Distribution Plan</span>
      </div>
      ...
      {htmlContent ? (
        ...
        <div className="card p-8 text-center text-gray-400">
          <p className="text-lg mb-2">No distribution plan yet</p>
          <p className="text-sm">Add a marketing/distribution plan via the API.</p>
        </div>
      )}
    </>
  );
