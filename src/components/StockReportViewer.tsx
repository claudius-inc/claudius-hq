...
export function StockReportViewer({ report }: { report: StockReport }) {
  ...
  const utcTimestamp = report.created_at 
  ...
  return (
    <>
      <ConfirmDialog
        ...
        title="Delete Report"
        description={`Are you sure you want to delete the ${report.ticker} report? This action cannot be undone.`}
        confirmLabel="Delete"
        variant="destructive"
      ...
      <div className={`bg-white border border-gray-200 rounded-xl p-5 group relative ${isDeleting ? "opacity-50" : ""}`}>
        ...
        <button
          ...
          disabled={isDeleting}
          className="absolute top-3 right-3 p-1.5 rounded transition-all text-gray-400 hover:text-red-600 hover:bg-red-50 opacity-0 group-hover:opacity-100"
          title="Delete report"
        >
          {isDeleting ? (
            <Spinner size="sm" />
